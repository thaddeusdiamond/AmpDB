...
class ArrayContainerIterator :
        public std::iterator<std::forward_iterator_tag, typename ContainerType::value_type> {
public:
...
/** A templated circular buffer based on std::vector. It holds an infinite number of items in an
array. This is a similar idea as java.util.ArrayDeque, with an interface similar to std::deque. */
template <typename T>
...
public:
    ...
    typedef base::ArrayContainerIterator<CircularBuffer > iterator;
